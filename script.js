  ...
  var walls = false;
  //here true turns the wall on that means the game reset when ther snake touches the wall.
  //You can turn it false to make the wall volatile
  ...
  var game = {
   ...
   //     ctx.fillRect(0, 0, canv.width, canv.height);
    ...
    loop: function () {
      ...
      ctx.fillStyle = '#ff094d';
      ctx.fillRect(fruit.x * gridSize+1, fruit.y * gridSize+1, gridSize-2, gridSize-2);

      if(stopped) {
        ctx.fillStyle = 'rgba(250,250,250,0.8)';
        ctx.font = "small-caps bold 10px Helvetica";
        ctx.fillText("press ARROW KEYS to START...", 24, 374);
      }

      ctx.fillStyle = 'white';
      ctx.font = "bold small-caps 10px Helvetica";
      ctx.fillText("points: " + points, 350, 15);
      ctx.fillText("top: " + pointsMax, 350, 30);
...
Snake.start(8);   ///this is for speed
Snake.setup.keyboard(false); // this makes the keyboard active
Snake.setup.fixedTail(false); // this is for making the snake longer